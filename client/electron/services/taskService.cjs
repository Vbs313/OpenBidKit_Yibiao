...
const { createTaskRecovery } = require('./tasks/taskRecovery.cjs');
const { createTaskSnapshots } = require('./tasks/taskSnapshots.cjs');
...
function createTaskService({ aiService, agentService, autoConfirmationService, technicalPlanStore, rejectionCheckStore, duplicateCheckStore, feasibilityReportStore, knowledgeBaseService, duplicateCheckService, openXmlHelperService, complianceCheckStore, complianceCheckerService, complianceModelService }) {
  ...
  // 任务快照：实现见 tasks/taskSnapshots.cjs。
  const {
    buildTechnicalPlanSnapshot,
    buildFeasibilityReportSnapshot,
    buildSnapshot,
  } = createTaskSnapshots({ copyPatchFields, hasOwn, isActiveTaskStatus, getTaskField });
  ...
  // 中断恢复：实现见 tasks/taskRecovery.cjs；下面是启动期的组合与调用顺序。
  const {
