...
- 评标办法 / 评审办法 / 磋商办法 / 综合评分法 / 详细评审
- 评分表 / 评分因素表 / 评分标准表 / 技术部分评分表
- 评审办法前附表、技术分值构成、技术部分（XX 分）

重点识别上述章节及“技术评分”“评分标准”“技术参数”“技术要求”“技术方案”“技术部分”“评审要素”等内容，不要提取商务、价格、资质等无关条目。

分类原则：
1. 技术评分项：指供应商/投标人需要在技术方案中一一响应、展开编写，并可对应形成技术方案章节的具体评分内容。评分表中“技术部分”下的每一行评分因素（如实施方案、服务保障、人员配置、培训、运维、应急等）都应作为独立的技术评分项；即使原文是表格，也要拆成条目。
2. 技术评分要求：指用于约束评分、解释评分、定义扣分或判定规则的通用规则或说明，例如符合性要求、偏离扣分规则、判定口径、适用范围说明、表后说明、通用评审规则。
3. 判断依据是该内容是否要求投标人在技术方案中展开具体方案内容；如果不是具体方案内容，即使带有分值或扣分规则，也归入技术评分要求。
4. 若原文存在层级关系，请保持顺序和来源，不要自行合并不相关条款。
5. 表格行请逐条提取，不要把整张评分表压成一段笼统描述。

输出格式（必须严格使用下列两个二级标题，后续目录生成依赖“技术评分项”段落）：
