...
function createTechnicalPlanStore({ app, db, fileService, agentService, taskLogStore, configStore }) {
  ...
  // 技术标元数据与待选招标文件清理：实现见 stores/tenderMeta.cjs。
  const {
    ensureMetaRow,
    readMetaRow,
    updateMeta,
  ...
  } = createTenderMeta({ db, tenderMarkdownPath, tenderMarkdownRelativePath });
  ...
  // 正文与配图计划持久化：实现见 stores/contentPersistence.cjs。
  const {
    loadContentSections,
    saveContentSections,
    saveContentPlans,
  ...
  // 任务与运行时持久化：实现见 stores/taskPersistence.cjs。
  const {
    saveTask,
    taskFromRow,
