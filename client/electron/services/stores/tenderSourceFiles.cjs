...
function createTenderSourceFiles(deps) {
  ...
  function loadTenderSourceFiles(meta = readMetaRow()) {
    ...
    if (Array.isArray(sourceFiles) && sourceFiles.length) {
      return sourceFiles.map((file) => ({
        ...
        quality: file.quality && typeof file.quality === 'object' ? file.quality : undefined,
  ...
  function writeTenderSourceMarkdown(source, index) {
    ...
    return {
      ...
      quality: source?.quality && typeof source.quality === 'object' ? source.quality : undefined,
      sourcePath: source?.source_path ? String(source.source_path) : undefined,
