...
def available_checks() -> list[dict[str, str]]:
    return [
        {"check_id": check_id, "check_name": item["name"]}
...
def run_checks(input_data: dict[str, Any]) -> list[dict[str, Any]]:
    requested = input_data.get("checks") or [PRICING_ARITHMETIC_ID]
    if not isinstance(requested, list) or not requested:
        requested = [PRICING_ARITHMETIC_ID]
    ...
    for raw_check_id in requested:
        ...
        if not check:
            ...
            continue
        try:
            results.append(check["runner"](input_data))
        except Exception as error:  # noqa: BLE001 - 单个检查失败不能中断后续检查
