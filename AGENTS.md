...
- 功能代码放 `src/features/<feature>/`；跨功能运行时代码放 `src/shared/`。`shared` **不得引用 feature**（`verify:module-graph` 硬失败）；跨端领域契约类型统一放 `src/shared/types/domains/`，不要在 feature 里定义再让 `shared/types` 反向引用。
...
## 聚焦验证
- 默认门禁（`cd client`）：`npm test` → `npm run verify:module-graph` → `npm run build`。
- 改 Renderer 页面/组件/Hook：上述三条 + 对应的 `smoke:<feature>-ui`（现役：`settings-ui` / `rejection-ui` / `technical-plan-ui` / `feasibility-ui` / `export-format-ui` / `knowledge-base-ui` / `pages-mount` / `compliance-ui`）。UI 冒烟必须基于 `npm run build` 的产物；只有 `compliance-ui` 需要先 `npm run dev`。
- 改 Electron Main/preload：先对改动的 `.cjs` 跑 `node --check`，再跑上述默认门禁；改服务装配或懒初始化时补 `npm run smoke:ipc-boot`。
- 改 SQLite Store、migration 或 native 模块：默认门禁 + `npm run smoke:electron-native` / `npm run smoke:schema`；需要 Electron ABI 的套件用 `npm run test:electron-abi`。
- 改依赖：`cd client; npm audit`。
- `npm run build` 可能只有既有 chunk 体积警告；不要把它当失败，除非命令退出非 0。

## 结构重构约定
- 动手前先用 `codex-range-io` 量「（注入数 × 输出数）÷ 行数」；**低于约 5 行每接口名不动刀**（样例：已切的 5.2–14，已留的 3.1–5.2）。
- 顺序固定：**先补 UI 冒烟 → 逐字等价搬迁 → 过默认门禁 → 把账记进 handover**。
- 搬迁必须做「逐字等价断言」：把 HEAD 的旧函数体与搬走后的新位置归一化比对，只放过显式声明的改写（JSX 搬家用「去掉 `props.` 后逐字一致」）。
- 保留也必须给判据，三条任选：① 闭包工厂包住整个领域；② 共享可变状态数 × 回调链长 > 拆分收益；③ 纯注册器 / 纯类型契约 / 长文件小函数领域库。
